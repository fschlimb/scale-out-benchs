...
def run_pd_workflow(quarter=1, year=2000, perf_file="", **kwargs):
    ...
    acq_pdf = acq_pdf.drop(['seller_name'], axis=1)
    acq_pdf['seller_name'] = acq_pdf['new']
    acq_pdf = acq_pdf.drop(['new'], axis=1)
    pdf = perf_df_tmp
...
def pd_load_names(**kwargs):
    ...
    return ddf.read_csv("mortgage/names.csv", names=cols, delimiter='|', dtype=dtypes, assume_missing=True)
...
def create_ever_features(pdf, **kwargs):
    ...
    return everdf.drop(['current_loan_delinquency_status'], axis=1)
...
def create_12_mon_features(joined_df, **kwargs):
    ...
    for y in range(1, n_months + 1):
        ...
        grp = tmpdf.groupby(['loan_id', 'josh_mody_n']).agg({'delinquency_12': 'max','upb_12': 'min'}).reset_index()
